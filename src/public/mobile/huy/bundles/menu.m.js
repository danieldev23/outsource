/* 縮小失敗。正在傳回未縮小的內容。
(760,21-22): run-time error JS1005: Expected '(': {
(762,9-10): run-time error JS1006: Expected ')': }
(762,9-10): run-time error JS1008: Expected '{': }
(762,10-11): run-time error JS1195: Expected expression: ,
(763,29-30): run-time error JS1010: Expected identifier: (
(770,21-22): run-time error JS1005: Expected '(': {
(772,9-10): run-time error JS1006: Expected ')': }
(772,9-10): run-time error JS1008: Expected '{': }
(774,2-3): run-time error JS1195: Expected expression: )
(808,1): run-time error JS1009: Expected '}'
(808,1): run-time error JS1107: Expecting more source characters
(753,20-31): run-time error JS1301: End of file encountered before function is properly closed: function ()
(808,1): run-time error JS1107: Expecting more source characters
(808,1): run-time error JS1009: Expected '}'
(730,15-26): run-time error JS1301: End of file encountered before function is properly closed: function ()
(808,1): run-time error JS1107: Expecting more source characters
(808,1): run-time error JS1006: Expected ')'
 */
$(document).ready(function() {
    ...
    // });
    //公告/活動贈點
    $(document).on("click", ".news_listT", function() {
        $(this).toggleClass("on");
        $(".news_listT").not($(this)).removeClass("on");
        $(".news_listIn").not($(this).next(".news_listIn")).slideUp(200);
        if ($(this).hasClass('on')) {
            $(this).next(".news_listIn").slideDown(200, function() {
                $("html,body").animate({
                    scrollTop: $(this).parent(".news_list")[0].offsetTop - 80
                }, 300);
            });
        } else {
            $(this).next(".news_listIn").slideUp(200);
        }
    });
    ...
    if (jQuery("#GameMainMenu").val()) {
        ...
        swiper1.slides.on('touchmove', function(e) {
            ...
            var touchesDiff = touchCurrent - touchStart; /*第一次觸摸與當前觸摸 的差距*/
            var slide = this;
            var onlyScrolling = /*判斷是否滾動*/
                (slide.scrollHeight > slide.offsetHeight) && //當slide可滾動時才允許
                (
                    (touchesDiff < 0 && startScroll === 0) || //從上邊緣開始滾動到底部
                    (touchesDiff > 0 && startScroll === (slide.scrollHeight - slide.offsetHeight)) || //從底部邊緣開始滾動到頂部
                    (startScroll > 0 && (startScroll < (slide.scrollHeight - slide.offsetHeight) &&
                        (slide.scrollHeight < (slide.clientHeight + slide.scrollTop - 10) || touchesDiff > 0))) //從中間開始
                );
            if (onlyScrolling) {
    ...
    if ($('.noColor').length > 0) {
        if ($(window).scrollTop() > 10) {
            $('.bg_header.noColor').addClass('black');
        } else {
            $('.bg_header.noColor').removeClass('black');
        }
...
$(document).ready(function() {
    ...
    $(".pMail_listTitle").off();
    $("div.pMail").on("click", ".pMail_listTitle", function() {
        var pMail_listT = $(this).parent('.pMail_listT');

        pMail_listT.parent('.pMail_list').addClass("read");
        pMail_listT.toggleClass("on");
        $(".pMail_listT").not(pMail_listT).removeClass("on");
        $(".pMail_listIn").not(pMail_listT.next(".pMail_listIn")).slideUp(200);
        if (pMail_listT.hasClass('on')) {
            pMail_listT.next(".pMail_listIn").slideDown(200, function() {
                $("html,body").animate({
                    scrollTop: pMail_listT.parent(".pMail_list")[0].offsetTop - 80
                }, 300);
            });
        } else {
            pMail_listT.next(".pMail_listIn").slideUp(200);
        }
    });
